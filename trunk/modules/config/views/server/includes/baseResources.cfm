...
<script type="text/javascript">
	function confirmDeleteBaseResource(type, index) {
		if(confirm("Delete base resource?")) {
			document.location = "index.cfm?event=config.ehSettings.doDeleteBaseResource&type=" + type + "&index=" + index;
		}
	}
</script>
		...
		<td colspan="2">
				...
				<cfloop list="#lstBaseResourceTypes#" index="resType">
					...
					<cfloop from="1" to="#arrayLen(aRes)#" index="i">
						<tr <cfif index mod 2>class="altRow"</cfif> <cfif baseResourceEditIndex eq i>style="font-weight:bold;"</cfif>>
							...
							<td><a href="index.cfm?event=config.ehSettings.dspMain&baseResourceEditIndex=#i#&baseResourceEditType=#resType#">#aRes[i]#</a></td>
							<td align="center">
								<a href="index.cfm?event=config.ehSettings.dspMain&baseResourceEditIndex=#i#&baseResourceEditType=#resType#"><img src="images/page_edit.png" border="0" alt="Edit base resource" title="Edit base resource"></a>
			...
			<cfif baseResourceEditIndex eq 0>
				<form name="frmAddBaseResource" action="index.cfm" method="post">
					<input type="hidden" name="event" value="config.ehSettings.doAddBaseResource">
					<table style="width:100%;border:1px solid silver;margin-top:5px;">
						<tr>
							...
							<td style="width:100px;" align="center">
								...
								<input type="button" name="btnCancel" value="Cancel" style="font-size:11px;" onclick="document.location='index.cfm?event=config.ehSettings.dspMain'">
							</td>
						</tr>
					</table>
				</form>
			<cfelseif baseResourceEditIndex gt 0>
				...
				<form name="frmEditBaseResource" action="index.cfm" method="post">
					<input type="hidden" name="event" value="config.ehSettings.doSaveBaseResource">
			...
			<cfelse>
				<br>
				<a href="index.cfm?event=config.ehSettings.dspMain&baseResourceEditIndex=0">Click Here</a> to add a base resources
			</cfif>
		</td>
	</tr>
