...
<cfscript>
	...
	stContRenders = oHomePortalsConfigBean.getContentRenderers();
...
<script type="text/javascript">
	function confirmDeleteContentRenderer(name) {
		if(confirm("Delete content renderer?")) {
			document.location = "index.cfm?event=config.ehSettings.doDeleteContentRenderer&name=" + name;
		}
	}
</script>
		...
		<td colspan="2">
			...
			<cfif contentRendererEditKey eq "__NEW__">
				<form name="frmAddContentRenderer" action="index.cfm" method="post">
					<input type="hidden" name="event" value="config.ehSettings.doSaveContentRenderer">
			...
			<cfelseif contentRendererEditKey neq "">
				<cftry>
					<cfset path = oHomePortalsConfigBean.getContentRenderer(contentRendererEditKey)>
					<cfcatch type="any">
						<cfset path = "">
					</cfcatch>
				</cftry>
