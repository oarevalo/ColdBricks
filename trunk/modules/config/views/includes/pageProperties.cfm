...
<cfscript>
	...
	stProperties = oHomePortalsConfigBean.getPageProperties();
...
<script type="text/javascript">
	function confirmDeleteProperty(name) {
		if(confirm("Delete page property?")) {
			document.location = "index.cfm?event=config.ehSettings.doDeleteProperty&name=" + name;
		}
	}
</script>
		...
		<td colspan="2">
			...
			<cfif propertyEditKey eq "__NEW__">
				<form name="frmAddProperty" action="index.cfm" method="post">
					<input type="hidden" name="event" value="config.ehSettings.doSaveProperty">
			...
			<cfelseif propertyEditKey neq "">
				<cftry>
					<cfset value = oHomePortalsConfigBean.getPageProperty(propertyEditKey)>
					<cfcatch type="any">
						<cfset value = "">
					</cfcatch>
				</cftry>
