...
<cfoutput>
		...
		<tr>
			...
			<td align="right" nowrap="yes">
				<form name="frmSearch" method="post" action="index.cfm" style="padding:0px;margin:0px;">
					...
					<input type="button" value="Search" style="width:auto;" onclick="doFormEvent('pages.ehPages.dspNode','nodePanel',this.form)">
				</form>
			</td>
			<td align="right">
	...
	<div style="background-color:##fff;height:399px;border:1px dashed ##ccc;margin-top:3px;overflow:auto;margin-bottom:3px;">
		<form name="frmDelete" method="post" action="index.cfm" style="padding:0px;margin:0px;">
			<input type="hidden" name="event" value="pages.ehPages.doDeleteNodes">
			<input type="hidden" name="path" value="#path#">
	...
	<div class="pagingControls">
		...
		Page 
			<select name="pageJump" style="font-size:10px;" onchange="doEvent('pages.ehPages.dspNode','nodePanel',{startRow:(this.value-1)*#rowsPerPage#+1,searchTerm:'#jsstringformat(searchTerm)#,path:'#path#'})">
		...
		of #numPages# &nbsp;&middot;&nbsp;
		<cfif startRow gt 1>
			<a href="##" onclick="doEvent('pages.ehPages.dspNode','nodePanel',{startRow:1,searchTerm:'#jsstringformat(searchTerm)#',path:'#path#'})">First</a> &nbsp;&middot;&nbsp;
			<a href="##" onclick="doEvent('pages.ehPages.dspNode','nodePanel',{startRow:#prevPageStartRow#,searchTerm:'#jsstringformat(searchTerm)#',path:'#path#'})">Previous</a>
		<cfelse>
