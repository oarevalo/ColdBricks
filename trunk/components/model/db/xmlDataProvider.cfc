<cfcomponent extends="dataProvider">
	...
	<cffunction name="readXMLDoc" access="private" returntype="xml">
		...
		<cfscript>
			var xmlDoc = 0;
			var tableName = arguments._mapTableInfo.tableName;
			var xmlDocURL = variables.oConfigBean.getDataRoot() & "/" & tableName & ".xml.cfm";
			
			if(fileExists(expandPath(xmlDocURL)))
				xmlDoc = xmlParse(_fileRead(expandPath(xmlDocURL)));
			else {
				xmlDoc = xmlNew();
				xmlDoc.xmlRoot = xmlElemNew(xmlDoc,"data");
			}
	...
	<cffunction name="writeXMLDoc" access="private" returntype="void">
		...
		<cfset var xmlDocURL = variables.oConfigBean.getDataRoot() & "/" & tableName & ".xml.cfm">
		<cfset _fileWrite(expandPath(xmlDocURL), toString(arguments.xmlDoc))>
	</cffunction>
	...
	<cffunction name="_fileRead" access="private" returntype="string">
	...
	<cffunction name="_fileWrite" access="private" returntype="void">
		<cfargument name="path" type="string" required="true">
