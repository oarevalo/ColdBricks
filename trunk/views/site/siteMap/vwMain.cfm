...
<cfsavecontent variable="tmpHTML">
	...
	<script type="text/javascript">
		function selectTreeNode(path) {
			doEvent("ehSiteMap.dspNode","nodePanel",{path: path});
		}
		
		function deleteNode(path) {
			if(confirm("Delete node?")) {
				var loc = "index.cfm?event=ehSiteMap.doDeleteNode&path=" + path;
				document.location = loc;
			}
		}
		
		function reloadNode(path,account) {
			doEvent("ehSiteMap.dspNode","nodePanel",{path: path,account: account});
		}
		
		function loadTreeNode(id,path) {
			doEvent('ehSiteMap.dspTreeNode',id,{path:path});
		}
	</script>
</cfsavecontent>
