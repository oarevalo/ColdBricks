...
function deletePageProperty(name) {
	if(confirm('Delete property?')) {
		document.location = '?event=ehPage.doRemovePageProperty&propName='+name;
	}
}
</script>
		...
		<td>
				...
				<table class="cp_dataTable">
					...
					<cfloop collection="#stProps#" item="key">
						<tr <cfif i mod 2>style="background-color:##f3f3f3;"</cfif>>
							...
							<td>#stProps[key]#</td>
							<td align="right">
								<a href="index.cfm?event=ehPage.dspPageProperties&propName=#key#"><img src="images/page_edit.png" align="absmiddle" border="0"></a>
			...
			<form name="frm" method="post" action="index.cfm" style="margin:0px;padding:0px;">
				<fieldset style="margin-top:10px;border:1px solid ##ccc;background-color:##ebebeb;">
					...
					<input type="hidden" name="event" value="ehPage.doSetPageProperty">
					...
					<table cellspacing="0" cellpadding="2" style="margin-bottom:5px;">
						...
						<tr valign="top">
							...
							<td>
								...
								<cfif propName neq "">
									<input type="button" name="btnCancel" value="Cancel" onclick="document.location='?event=ehPage.dspPageProperties'">	
								</cfif>
							</td>
						</tr>
					</table>
