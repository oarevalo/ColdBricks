...
<script type="text/javascript">
	function confirmDeleteTemplate(name) {
		if(confirm("Delete template?")) {
			document.location = "index.cfm?event=ehSiteConfig.doDeleteRenderTemplate&name=" + name;
		}
	}
</script>
		...
		<td colspan="2">
			...
			<table style="width:100%;border:1px solid silver;">
				...
				<cfloop collection="#stTemplates#" item="key">
					<tr <cfif index mod 2>class="altRow"</cfif>>
						...
						<td style="width:100px;" align="center">#stTemplates[key].type#</td>
				...
				<cfset stTemplates = oAppConfigBean.getRenderTemplates()>
				<cfloop collection="#stTemplates#" item="key">
			...
			<cfif templateEditKey neq "">
				<cfif templateEditKey neq "__NEW__">
					<cfset isNew = false>
					<cfset stTemplate = oAppConfigBean.getRenderTemplate(templateEditKey)>
				<cfelse>
					<cfset isNew = true>
					...
					<tr valign="top">
						<td style="width:430px;">
							<form name="frmEditTemplate" action="index.cfm" method="post">
								<input type="hidden" name="event" value="ehSiteConfig.doSaveRenderTemplate">
								<cfif not isNew>
