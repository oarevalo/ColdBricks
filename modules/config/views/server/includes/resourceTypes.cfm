...
<cfscript>
	...
	stResTypes = oHomePortalsConfigBean.getResourceTypes();
...
<script type="text/javascript">
	function confirmDeleteResType(name) {
		if(confirm("Delete resource type?")) {
			document.location = "index.cfm?event=config.ehSettings.doDeleteResourceType&name=" + name;
		}
	}
	function confirmDeleteResTypeProp(type,name) {
		if(confirm("Delete resource type property?")) {
			document.location = "index.cfm?event=config.ehSettings.doDeleteResourceTypeProperty&resTypeEditKey=" + type + "&name="+name;
		}
	}
		...
		<td colspan="2">
			...
			<cfif resTypeEditKey neq "">
				<cfif resTypeEditKey neq "__NEW__">
					<cfset isNew = false>
					<cfset stResType = oHomePortalsConfigBean.getResourceType(resTypeEditKey)>
				<cfelse>
					<cfset isNew = true>
					...
					<tr valign="top">
						<td style="width:430px;">
							<form name="frmEditResType" action="index.cfm" method="post">
								<input type="hidden" name="event" value="config.ehSettings.doSaveResourceType">
								<cfif not isNew>
							...
							<td>
								...
								<cfif resTypePropEditKey gte 0>
									...
									<form name="frmEditResTypeProp" action="index.cfm" method="post">
										<input type="hidden" name="event" value="config.ehSettings.doSaveResourceTypeProperty">
