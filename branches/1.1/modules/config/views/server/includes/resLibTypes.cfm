...
<cfscript>
	...
	stResTypes = oHomePortalsConfigBean.getResourceLibraryTypes();
...
<script type="text/javascript">
	function confirmDeleteResType(name) {
		if(confirm("Delete resource library type ?")) {
			document.location = "index.cfm?event=config.ehSettings.doDeleteResourceLibraryType&prefix=" + name;
		}
	}
	function confirmDeleteResTypeProp(type,name) {
		if(confirm("Delete resource library type  property?")) {
			document.location = "index.cfm?event=config.ehSettings.doDeleteResourceLibraryTypeProperty&resLibTypeEditKey=" + type + "&name="+name;
		}
	}
	function togglePropType(type) {
		var s1 = document.getElementById("rowValues");
		var s2 = document.getElementById("rowResourceLibraryTypes");
					...
					<tr valign="top">
						<td style="width:430px;">
							<form name="frmEditResType" action="index.cfm" method="post">
								<input type="hidden" name="event" value="config.ehSettings.doSaveResourceLibraryType">
								<cfif not isNew>
							...
							<td>
								...
								<cfif resLibTypePropEditKey gte 0>
									...
									<form name="frmEditResTypeProp" action="index.cfm" method="post">
										<input type="hidden" name="event" value="config.ehSettings.doSaveResourceLibraryTypeProperty">
