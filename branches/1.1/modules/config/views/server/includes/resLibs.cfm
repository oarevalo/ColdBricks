...
<script type="text/javascript">
	function confirmDeleteResLibPath(index) {
		if(confirm("Delete resource library path?")) {
			document.location = "index.cfm?event=config.ehSettings.doDeleteResLibPath&index=" + index;
		}
	}
</script>
		...
		<td colspan="2">
			...
			<table style="width:100%;border:1px solid silver;">
				...
				<cfloop from="1" to="#arrayLen(aResLibs)#" index="i">
					<tr <cfif i mod 2>class="altRow"</cfif> <cfif resLibPathEditIndex eq i>style="font-weight:bold;"</cfif>>
						...
						<td><a href="index.cfm?event=config.ehSettings.dspMain&resLibPathEditIndex=#i#">#aResLibs[i]#</a></td>
						<td align="center">
							<a href="index.cfm?event=config.ehSettings.dspMain&resLibPathEditIndex=#i#"><img src="images/page_edit.png" border="0" alt="Edit resource library path" title="Edit resource library path"></a>
			...
			<cfif resLibPathEditIndex eq 0>
				<form name="frmAddResLibPath" action="index.cfm" method="post">
					<input type="hidden" name="event" value="config.ehSettings.doAddResLibPath">
			...
			<cfelseif resLibPathEditIndex gt 0>
				...
				<form name="frmEditResLibPath" action="index.cfm" method="post">
					<input type="hidden" name="event" value="config.ehSettings.doSaveResLibPath">
			...
			<cfelse>
				<br>
				<a href="index.cfm?event=config.ehSettings.dspMain&resLibPathEditIndex=0">Click Here</a> to add a resource library path
			</cfif>
		</td>
	</tr>
