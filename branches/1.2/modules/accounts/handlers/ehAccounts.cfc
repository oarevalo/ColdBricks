	...
	<cffunction name="dspAddPage" access="public" returntype="void">
		<cfscript>
			...
			try {
				...
				setMessage("error",e.message);
				getService("bugTracker").notifyService(e.message, e);
				setNextEvent("accounts.ehAccounts.dspMain");
			}
		</cfscript>
	</cffunction>
	...
	<cffunction name="dspTokenizePage" access="public" returntype="void">
		<cfscript>
			...
			try {
				...
				setView("vwTokenizePage");		

			} catch(any e) {
				setMessage("error",e.message);
				getService("bugTracker").notifyService(e.message, e);
				setNextEvent("accounts.ehAccounts.dspAddPage");
			}
		</cfscript>
	</cffunction>
	...
	<cffunction name="doSave" access="public" returntype="void">
		<cfscript>
			...
			try {
				...
				if(accountID neq "") 
					setNextEvent("accounts.ehAccounts.dspEdit","accountID=#accountID#");
				else
					setNextEvent("accounts.ehAccounts.dspCreate");
	...
	<cffunction name="doCopyTokenizedPage" access="public" returntype="void">
		<cfscript>
			...
			try {
				...
				setView("vwTokenizationStatus");
				
			} catch(coldBricks.validation e) {
				setMessage("warning",e.message);
				setNextEvent("accounts.ehAccounts.dspTokenizePage");
